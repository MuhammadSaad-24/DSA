...
typename DLL<Type>::Iterator DLL<Type>::Begin()
{
    if(Head)
        return Iterator(Head);
    else
    {
        cout << "Error::No Node Exist To Point Head\n";
        return nullptr;
    }
}

template <typename Type> 
typename DLL<Type>::Iterator DLL<Type>::End()
{
    if(Tail)
...
public:
    ...
    bool operator>(const Type&) const;
    template<typename T>
    friend bool operator>(const T&,const ListIterator&);
    template<typename T>
    friend bool operator<(const T&,const ListIterator&);
    Type& operator*();
...
{
    if(Iptr)
        return Iptr->Data < other.Iptr->Data;
}
...
template <typename Type> 
bool DLL<Type>::ListIterator::operator<(const Type& other) const
{
    if(Iptr)
        return Iptr->Data < other;
}
...
template<typename T>
bool operator>(const T& data,const typename DLL<T>::ListIterator& other)
{
    if(other.Iptr)
        return data > other.Iptr->Data;
}
...
int main()
{
    ...
    DLL<int>::Iterator  i = l1.Begin();

    cout << "Traverse Through Iterators : ";
    
    for(i = l1.Begin();i != nullptr;i++)
    {
        if(i != l1.End())
            cout << *i << "->";
        else
